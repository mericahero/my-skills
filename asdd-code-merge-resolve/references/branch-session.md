...
4. 仍无法确定时，创建 `docs/merge-reports/{YYYYMMDD-HHmm}-{current-branch}-code-merge/`。

## 分支输入

支持：

- `分析 feature/a 分支合并到 develop 分支后的代码冲突`
- `source=feature/a target=develop`
- `feature/a -> develop`

本 skill 处于 merge/rebase 后阶段，source/target 主要用于报告归档和证据追踪；不得切换分支。

## metadata

如果 session 下不存在 `metadata.yml`，创建最小 metadata：
...
reports:
  code_conflicts: "code-conflicts.md"
```
